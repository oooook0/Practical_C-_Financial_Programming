...
VolatilityCalculator::VolatilityCalculator(const VolatilityCalculator &v) : m_prices(v.m_prices)
{
...
double VolatilityCalculator::std()
{
	double m = mean();
	double val_sum = 0;
	for (int i =0; i < m_prices.size(); ++i)
	{
		val_sum += pow((m_prices[i] -m), 2);
	}
	return sqrt(val_sum/ (m_prices.size() -1));
}
